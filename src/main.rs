...
fn main() {
    ...
    if !args.json {
        ...
        eprintln!("[INFO] Parallel scan with {} thread(s){}", threads, mode_tags);
        if mem_capped {
    ...
    if let Some((path, ax_exp, bfs_exp)) = &path_result {
        ...
        // Avoid an unused-mut warning when start/goal match.
        let _ = &mut start;
    }
    ...
    print_summary_box(&args, threads, elapsed, stats);
}
