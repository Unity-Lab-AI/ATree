...
impl ScopeResolutionIndexes {
    ...
    /// Merge another index shard into this one.
    /// Used to combine per-thread index shards after parallel build.
    pub fn merge(&mut self, other: ScopeResolutionIndexes) {
        // Merge bindings
        for (scope_id, name_map) in other.bindings {
            let entry = self.bindings.entry(scope_id).or_default();
            for (name, refs) in name_map {
                entry.entry(name).or_default().extend(refs);
            }
        }
        // Merge binding augmentations
        for (scope_id, name_map) in other.binding_augmentations {
            let entry = self.binding_augmentations.entry(scope_id).or_default();
            for (name, refs) in name_map {
                entry.entry(name).or_default().extend(refs);
            }
        }
        // Merge type bindings
        for (scope_id, name_map) in other.type_bindings {
            let entry = self.type_bindings.entry(scope_id).or_default();
            for (name, tb) in name_map {
                entry.entry(name).or_insert(tb);
            }
        }
        // Merge method dispatch
        self.method_dispatch.extend(other.method_dispatch);
        // Merge parsed files
        self.parsed_files.extend(other.parsed_files);
        // Merge scopes
        self.scopes_by_id.extend(other.scopes_by_id);
        for (path, ids) in other.scopes_by_file {
            self.scopes_by_file.entry(path).or_default().extend(ids);
        }
        // Merge symbols
        self.symbols_by_id.extend(other.symbols_by_id);
        for (path, syms) in other.symbols_by_file {
            self.symbols_by_file.entry(path).or_default().extend(syms);
        }
        // Merge imports
    ...
    pub fn find_owned_member(&self, owner_symbol_id: u64, member_name: &str) -> Option<&Symbol> {
        self.symbols_by_id.values().find(|&sym| sym.owner_id == Some(owner_symbol_id) && sym.name == member_name).map(|v| v as _)
    }
...
pub struct ResolvedReference {
    ...
    /// Coarse confidence tier (legacy enum, kept for compatibility).
    pub confidence: Confidence,
    /// Fine-grained evidence-based confidence (0.0–1.0), if computed by the
    /// registry-based resolution pipeline. When present, takes precedence over
    /// `confidence` for edge emission.
